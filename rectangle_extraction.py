...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct  8 08:10:05 2023
...
def segment_chars(plate_img, fixed_width):
    ...
    thresh = cv2.adaptiveThreshold(V, maxValue=255, adaptiveMethod=cv2.ADAPTIVE_THRESH_GAUSSIAN_C, thresholdType=cv2.THRESH_BINARY, blockSize=11, C=2)
    ...
    for label in np.unique(labels):
        ...
        cnts = cv2.findContours(labelMask, mode=cv2.RETR_EXTERNAL, method=cv2.CHAIN_APPROX_SIMPLE)
        ...
        contours, hier = cv2.findContours(charCandidates, mode=cv2.RETR_EXTERNAL, method=cv2.CHAIN_APPROX_SIMPLE)
